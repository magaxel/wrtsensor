...
Per-host CPU % over time using HA's built-in graph card. New host metric entities use the suggested ID pattern `sensor.wrtsensor_<ip_with_underscores>_cpu`.
...
Disk use per host as a static list. New host metric entities use the suggested ID pattern `sensor.wrtsensor_<ip_with_underscores>_disk`.
...
| `sensor.wrtsensor_<host>_cpu` | CPU % per host, e.g. `sensor.wrtsensor_192_0_2_1_cpu` |
| `sensor.wrtsensor_<host>_ram` | RAM % per host |
| `sensor.wrtsensor_<host>_disk` | Disk % per host |
